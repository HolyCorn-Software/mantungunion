...
# This file allows mantungunion to be run as a system-wide command

src="$(readlink -f $BASH_SOURCE)"
cd "$(dirname $src)"
appName=mantungunion
...
Possible options are: issue"
    ...
    service)
        ...
        case $2 in
            enable)
                ...
                #Then make this mantungunion.sh available everywhere
                enable_command
    ...
    update)
        ...
        sudo mv "$TLS_CERT.tmp" "$TLS_CERT"
        sudo mv "$TLS_KEY.tmp" "$TLS_KEY"
        sudo mv "$TLS_CA.tmp" "$TLS_CA"
